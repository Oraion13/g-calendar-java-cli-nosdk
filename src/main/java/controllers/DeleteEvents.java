...
public class DeleteEvents {
    ...
    private void deleteEvents(String from, String to, int limit) throws IOException {
        Map<Integer, TypeEvent> toDelete = eventsManagement.getEventsInMap(from, to, limit);
        // display the events
        System.out.println("Delete ID :   Event\n-----------------------------------");
        eventsManagement.printMapEvents(toDelete);

        int deleteID = ValidIOHandlers.getChoice("Enter a Delete ID to delete an event [Number / 0]: ");
